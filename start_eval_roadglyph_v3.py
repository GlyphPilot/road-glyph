...
def bash_file_bench2drive(job, port, tm_port, partition_name):
    ...
    # Assign GPU ID from port offset (cycles 0-7) so concurrent jobs each use a distinct GPU
    gpu_id = (port - 10000) // 50 % 8
...
# Avoid sourcing .bashrc to prevent environment conflicts
# source ~/.bashrc
...
# --- Explicit GPU assignment (port-based) ---
export CUDA_VISIBLE_DEVICES={gpu_id}

# --- set roots ---
export CARLA_ROOT={cfg["carla_root"]}
export WORK_DIR={cfg["repo_root"]}/Bench2Drive
export SCENARIO_RUNNER_ROOT=$WORK_DIR/scenario_runner
export LEADERBOARD_ROOT=$WORK_DIR/leaderboard

# --- reset & set PYTHONPATH (avoid conflicts) ---
export PYTHONPATH=""
...
export PYTHONPATH=$PYTHONPATH:{cfg["repo_root"]}
...
def get_running_jobs():
    ...
    cmd = f'squeue -u {user}'
    running_jobs = subprocess.check_output(cmd, shell=True).decode("utf-8").splitlines()
...
# ============================================================
# USER CONFIG — set these to match your environment
# ============================================================
REPO_ROOT   = "/path/to/this/repo"          # absolute path to this repository root
CARLA_ROOT  = "/path/to/carla0915"          # CARLA 0.9.15 installation directory
CHECKPOINT  = "/path/to/checkpoint.ckpt"    # trained RoadGlyphV3 checkpoint (.ckpt)
OUT_ROOT    = "/path/to/eval_output"        # directory where results will be written
ROUTE_PATH  = f"{REPO_ROOT}/Bench2Drive/leaderboard/data/bench2drive_split"
AGENT_FILE  = f"{REPO_ROOT}/team_code/agent_road_glyph_v3.py"
